...
namespace CsvHelper.Tests.Mappings.ConstructorParameter
{
	...
	public class NameIndexMapTests
    {
		...
		{
			var map = new DefaultClassMap<Foo>();
			map.Parameter(() => ConfigurationFunctions.GetConstructor(typeof(Foo)), "id").NameIndex(0);
			map.Parameter(() => ConfigurationFunctions.GetConstructor(typeof(Foo)), "name").NameIndex(1);
