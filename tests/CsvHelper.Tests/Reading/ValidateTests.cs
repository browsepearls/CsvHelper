...
namespace CsvHelper.Tests.Reading
{
	...
	public class ValidateTests
	{
		...
		private sealed class ValidateMap : ClassMap<Test>
		{
			public ValidateMap()
			{
				Map(m => m.Id).Validate(field => !string.IsNullOrEmpty(field.ToString()));
				Map(m => m.Name);
			}
		}
		...
		private sealed class LogInsteadMap : ClassMap<Test>
		{
			public LogInsteadMap(StringBuilder logger)
			{
				...
				{
					var isValid = !string.IsNullOrEmpty(field.ToString());
					if (!isValid)
					{
						logger.AppendLine($"Field '{field.ToString()}' is not valid!");
					}
